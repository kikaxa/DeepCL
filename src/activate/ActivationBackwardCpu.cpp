...
//        cerr << "input=" << inputs[i] << " deriv=" << fn->calcDerivative(inputs[i])
//            << " error=" << errors[i];
        gradInput[i] = fn->calcDerivative(outputs[i]) * gradOutput[i];
        cerr << " gradInput=" << gradInput[i] << endl;
    }
}
VIRTUAL void ActivationBackwardCpu::backward(int batchSize, 
    ...
    for(int i = 0; i < 4; i++) {
        cerr << "i=" << i << " outputs=" << outputs[i] << " gradOutput=" << gradOutput[i] << endl;
    }
    ...
    for(int i = 0; i < 4; i++) {
        cerr << "i=" << i << " gradInput=" << gradInput[i] << endl;
    }
